...
> Een gigabyte is gelijk aan 1.073.741.824 (oftewel 2^30) en in werkelijkheid gaat meerder bytes tegelijk ophalen (afhankelijk van je databus).  
...
Ook files, computerprogramma's, ... moet je bekijken in bytes

> Domme vragen bestaan niet, waarom  is 1 bytes 8 bits?  
...
In dat geval spreken we echter over 1000-tallen:
...
* De inhoud van een String (100, 101, 98, 117, 103, 103, 101, 110 is een array van bytes)
...
Om dit te doen wordt meestal het **volgende algoritme** toegepest:

* **Deel** het getal door **2** iteratief.
* Per iteratie
...
Een byte bestaande uit exact 2 nibbles kan je dan ook exact mappen naar een hexadecimaal getal.

Een byte kan altijd exact worden voorgesteld als 2 hexadecimale getallen.  
